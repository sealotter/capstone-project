              ...
              <div className='postoutline'>
                <div className="post">
                  ...
                  <div className="post_footer">
                    ...
                    }}>{comments.length}<ChatBubbleOutlineIcon fontSize="small" /></Button>
                    {post.likes.includes(auth.username)?<Button>{post.likes.length}<FavoriteIcon fontSize='small' onClick={()=>{updatePostLikes(post.id, auth.username)}}/></Button>:<Button>{post.likes.length}<FavoriteBorderIcon fontSize="small" onClick={()=>{updatePostLikes(post.id, auth.username)}}/></Button>}
                  </div>
                    ...
                    return(
                      <div key={comment.id} className="postOut">
                        <div className="post postcomment">
                          ...
                          <div className="post_footer">
                            {comment.likes.includes(auth.username)?<Button>{comment.likes.length}<FavoriteIcon fontSize='small' onClick={()=>{updatePostLikes(comment.id, auth.username)}}/></Button>:<Button>{comment.likes.length}<FavoriteBorderIcon fontSize="small" onClick={()=>{updatePostLikes(comment.id, auth.username)}}/></Button>}
                          </div>
                        </div>
                      </div>
                    )
