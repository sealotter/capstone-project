...
router.get('/page/:page', async(req, res, next)=>{
  try{
    ...
    if(search.nameSearch){
      media = (await axios.get(`https://api.themoviedb.org/3/search/${search.media}?api_key=${process.env.REACT_APP_MOVIE_KEY}&language=en-US&page=${page}&include_adult=false&query=${search.nameSearch}`)).data
    }else if(search.peopleSearch){
      media = (await axios.get(`https://api.themoviedb.org/3/search/${search.media}?api_key=${process.env.REACT_APP_MOVIE_KEY}&language=en-US&page=${page}&include_adult=false&query=${search.peopleSearch}`)).data
    }else{
      let searchString = ''
      for(key in search){
      ...
      media = (await axios.get(`https://api.themoviedb.org/3/discover/${search.media}?api_key=${process.env.REACT_APP_MOVIE_KEY }${searchString}&page=${page}`)).data
    }
    console.log(media)
    res.json(media)
  }catch(err){
